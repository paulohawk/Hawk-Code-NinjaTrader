...
{
    public class HawkMidasBands : Indicator
    {
        ...
        private Brush _barAnchorMedianBrush = Brushes.DarkGoldenrod;
        ...
        private Brush _bandCloudBrush = new SolidColorBrush(Media.Color.FromArgb(51, 128, 128, 128)); // 20%
        ...
        protected override void OnStateChange()
        {
            ...
            {
                Description = "Hawk MIDAS Bands (anchored VWAP High/Median/Low with cloud and Hawk panel).";
                Name = "HawkMidasBands";
                ...
                // Plots: High/Median/Low
                AddPlot(Brushes.DimGray, "VWAP_Bar_High");
                AddPlot(new Stroke(Brushes.DarkGoldenrod, DashStyleHelper.Dot, 1), PlotStyle.Line, "VWAP_Bar_Median");
                ...
                BandCloudBrush = new SolidColorBrush(Media.Color.FromArgb(51, 128, 128, 128));
                BandCloudOpacity = 20.0;

                // Anchor dot defaults
                BarAnchorHighColor = Brushes.DimGray;
                BarAnchorMedianColor = Brushes.DarkGoldenrod;
                BarAnchorLowColor = Brushes.DimGray;
            }
            else if (State == State.DataLoaded)
            {
                try { sessionIterator = new SessionIterator(Bars); }
                catch (Exception ex) { if (DebugMode) Log($"[HawkMidas] SessionIterator init failed: {ex.Message}", LogLevel.Warning); }
                ...
                if (EnableBarLogging && !string.IsNullOrEmpty(_logFolder))
                {
                    ...
                    catch (Exception ex) { if (DebugMode) Log($"[HawkMidas] Could not create log folder {_logFolder}: {ex.Message}", LogLevel.Warning); }
                }
                ...
                catch (Exception ex) { if (DebugMode) Log($"[HawkMidas] BuildSessionStarts failed in DataLoaded: {ex.Message}", LogLevel.Warning); }
            }
        ...
        protected override void OnBarUpdate()
        {
            ...
            if (targetSessionIndex < 0 || targetSessionIndex >= sessionStartIndices.Count)
            {
                ...
                else
                {
                    ...
                    {
                        Log($"[HawkMidas] targetSessionIndex {targetSessionIndex} out-of-range -> returning NaN (Fallback disabled)", LogLevel.Information);
        ...
        private void TryLogBarIfEnabled()
        {
            try
            {
                ...
                {
                    if (DebugMode) Log($"[HawkMidas] Failed to write log {filePath}: {ex.Message}", LogLevel.Warning);
                }
            }
            catch (Exception ex) { if (DebugMode) Log($"[HawkMidas] TryLogBarIfEnabled error: {ex.Message}", LogLevel.Error); }
        }
            ...
            try
            {
                ...
                if (anchorValid_byBar && anchorBarIndex_byBar >= 0 && anchorBarIndex_byBar < Bars.Count)
                {
                    ...
                    if (barsAgo >= 0 && barsAgo < Bars.Count)
                    {
                        ...
                        Draw.Dot(this, "BARANCH_M_" + anchorBarIndex_byBar, false, t, (High[barsAgo] + Low[barsAgo]) / 2.0, bMed ?? Brushes.DarkGoldenrod);
                        Draw.Dot(this, "BARANCH_L_" + anchorBarIndex_byBar, false, t, Low[barsAgo], bLow ?? Brushes.DimGray);
                    }
                }
            }
            catch (Exception ex) { if (DebugMode) Log($"[HawkMidas] Draw dots error: {ex.Message}", LogLevel.Error); }
        }
        ...
        private void DrawBandCloud()
        {
            try
            {
                ...
                var baseBrush = BandCloudBrush ?? new SolidColorBrush(Media.Color.FromArgb(51, 128, 128, 128));
                ...
                {
                    area = new SolidColorBrush(Media.Color.FromArgb(alpha, scb.Color.R, scb.Color.G, scb.Color.B));
                    area.Freeze();
                }
                else
                {
            ...
            catch (Exception ex) { if (DebugMode) Log($"[HawkMidas] DrawBandCloud error: {ex.Message}", LogLevel.Error); }
        }
        ...
        private void UpdatePlotBrushes()
        {
            ...
            catch (Exception ex) { if (DebugMode) Log($"[HawkMidas] UpdatePlotBrushes error: {ex.Message}", LogLevel.Error); }
        }
        ...
        private void LogReadyOnce()
        {
            ...
            if (!loggedReady)
            {
                Log("[HawkMidas] Ready: anchor valid and values computed", LogLevel.Information);
                loggedReady = true;
            }
        }
        ...
        private void LogNotReadyOnce(string reason)
        {
            ...
            if (!loggedNotReady)
            {
                Log($"[HawkMidas] Not ready: {reason}", LogLevel.Information);
                loggedNotReady = true;
            }
        }
        ...
        private void UpdateInfoText()
        {
            ...
            double highBand = Values[0][0];
            double midBand  = Values[1][0];
            double lowBand  = Values[2][0];
            double px       = Close[0];

            if (double.IsNaN(highBand) || double.IsNaN(midBand) || double.IsNaN(lowBand) || double.IsNaN(px))
            {
                _infoText = string.Empty;
                return;
            }

            string Fp(double v) => double.IsNaN(v) ? "-" : v.ToString("0.00");
            string Ft(double v) => double.IsNaN(v) ? "-" : v.ToString("0.0");

            string contexto, pressao;
            if (px > highBand)
            {
                contexto = "ACIMA";
                pressao = "pressão compradora";
            }
            else if (px < lowBand)
            {
                contexto = "ABAIXO";
                pressao = "pressão vendedora";
            }
            else
            {
                contexto = "DENTRO";
                pressao = "pressão neutra";
            }

            string clampInfo = AnchorWasClamped ? "CLAMP" : "OK";
            string readyStr = IsReady ? "OK" : "NÃO";
            string cloudStr = EnableBandCloud ? "ON" : "OFF";
            string opacityStr = BandCloudOpacity.ToString("0");

            string line1 = $"Midas Bands: Alta {Fp(highBand)} | Med {Fp(midBand)} | Baixa {Fp(lowBand)} | Preço {Fp(px)}";
            string line2 = $"Contexto: {contexto} — {pressao}";
            string line3 = $"Âncora: Sess {SessionOffset} | Bar {BarInSession} | {clampInfo}";
            string line4 = $"Status: {readyStr} | Nuvem: {cloudStr} ({opacityStr}%)";
        ...
        public string BandCloudBrushSerializable
        {
            ...
            set
            {
                ...
                catch { _bandCloudBrush = new SolidColorBrush(Media.Color.FromArgb(51, 128, 128, 128)); }
            }
        }
...
namespace NinjaTrader.NinjaScript.Indicators
{
	...
	{
		private HawkMidasBands[] cacheHawkMidasBands;
		public HawkMidasBands HawkMidasBands(int sessionOffset, int barInSession, bool useLastBarIfBarIndexTooLarge, bool plotOnlyNextSession, bool debugMode, bool anchorBySessionHighLow, bool fallbackToNearestSession, bool enableBarLogging, int logTimeframeMin, string logFolder, bool logPerSessionFile, bool enableBandCloud, double bandCloudOpacity)
		{
			return HawkMidasBands(Input, sessionOffset, barInSession, useLastBarIfBarIndexTooLarge, plotOnlyNextSession, debugMode, anchorBySessionHighLow, fallbackToNearestSession, enableBarLogging, logTimeframeMin, logFolder, logPerSessionFile, enableBandCloud, bandCloudOpacity);
		}

		public HawkMidasBands HawkMidasBands(ISeries<double> input, int sessionOffset, int barInSession, bool useLastBarIfBarIndexTooLarge, bool plotOnlyNextSession, bool debugMode, bool anchorBySessionHighLow, bool fallbackToNearestSession, bool enableBarLogging, int logTimeframeMin, string logFolder, bool logPerSessionFile, bool enableBandCloud, double bandCloudOpacity)
		{
			if (cacheHawkMidasBands != null)
				for (int idx = 0; idx < cacheHawkMidasBands.Length; idx++)
					if (cacheHawkMidasBands[idx] != null && cacheHawkMidasBands[idx].SessionOffset == sessionOffset && cacheHawkMidasBands[idx].BarInSession == barInSession && cacheHawkMidasBands[idx].UseLastBarIfBarIndexTooLarge == useLastBarIfBarIndexTooLarge && cacheHawkMidasBands[idx].PlotOnlyNextSession == plotOnlyNextSession && cacheHawkMidasBands[idx].DebugMode == debugMode && cacheHawkMidasBands[idx].AnchorBySessionHighLow == anchorBySessionHighLow && cacheHawkMidasBands[idx].FallbackToNearestSession == fallbackToNearestSession && cacheHawkMidasBands[idx].EnableBarLogging == enableBarLogging && cacheHawkMidasBands[idx].LogTimeframeMin == logTimeframeMin && cacheHawkMidasBands[idx].LogFolder == logFolder && cacheHawkMidasBands[idx].LogPerSessionFile == logPerSessionFile && cacheHawkMidasBands[idx].EnableBandCloud == enableBandCloud && cacheHawkMidasBands[idx].BandCloudOpacity == bandCloudOpacity && cacheHawkMidasBands[idx].EqualsInput(input))
						return cacheHawkMidasBands[idx];
			return CacheIndicator<HawkMidasBands>(new HawkMidasBands(){ SessionOffset = sessionOffset, BarInSession = barInSession, UseLastBarIfBarIndexTooLarge = useLastBarIfBarIndexTooLarge, PlotOnlyNextSession = plotOnlyNextSession, DebugMode = debugMode, AnchorBySessionHighLow = anchorBySessionHighLow, FallbackToNearestSession = fallbackToNearestSession, EnableBarLogging = enableBarLogging, LogTimeframeMin = logTimeframeMin, LogFolder = logFolder, LogPerSessionFile = logPerSessionFile, EnableBandCloud = enableBandCloud, BandCloudOpacity = bandCloudOpacity }, input, ref cacheHawkMidasBands);
		}
	}
}
...
namespace NinjaTrader.NinjaScript.MarketAnalyzerColumns
{
	...
	{
		public Indicators.HawkMidasBands HawkMidasBands(int sessionOffset, int barInSession, bool useLastBarIfBarIndexTooLarge, bool plotOnlyNextSession, bool debugMode, bool anchorBySessionHighLow, bool fallbackToNearestSession, bool enableBarLogging, int logTimeframeMin, string logFolder, bool logPerSessionFile, bool enableBandCloud, double bandCloudOpacity)
		{
			return indicator.HawkMidasBands(Input, sessionOffset, barInSession, useLastBarIfBarIndexTooLarge, plotOnlyNextSession, debugMode, anchorBySessionHighLow, fallbackToNearestSession, enableBarLogging, logTimeframeMin, logFolder, logPerSessionFile, enableBandCloud, bandCloudOpacity);
		}

		public Indicators.HawkMidasBands HawkMidasBands(ISeries<double> input , int sessionOffset, int barInSession, bool useLastBarIfBarIndexTooLarge, bool plotOnlyNextSession, bool debugMode, bool anchorBySessionHighLow, bool fallbackToNearestSession, bool enableBarLogging, int logTimeframeMin, string logFolder, bool logPerSessionFile, bool enableBandCloud, double bandCloudOpacity)
		{
			return indicator.HawkMidasBands(input, sessionOffset, barInSession, useLastBarIfBarIndexTooLarge, plotOnlyNextSession, debugMode, anchorBySessionHighLow, fallbackToNearestSession, enableBarLogging, logTimeframeMin, logFolder, logPerSessionFile, enableBandCloud, bandCloudOpacity);
		}
	}
